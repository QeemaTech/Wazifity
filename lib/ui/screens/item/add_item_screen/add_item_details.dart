...
import 'package:wazifati/ui/screens/item/add_item_screen/widgets/image_adapter.dart';

import 'package:wazifati/ui/screens/widgets/blurred_dialog_box.dart';
import 'package:wazifati/ui/screens/widgets/custom_text_form_field.dart';
import 'package:wazifati/ui/screens/widgets/dynamic_field.dart';
...
import 'package:wazifati/utils/image_picker.dart';
          ...
          bottomNavigationBar: Container(
            ...
            child: UiUtils.buildButton(context,
              ...
              if (_formKey.currentState?.validate() ?? false) {
                ...
                screenStack++;
                if (context.read<FetchCustomFieldsCubit>().isEmpty() ?? true) {
      ...
      return Wrap(
        children: [
          ...
          if (file != null || titleImageURL.isNotEmpty)
            uploadPhotoCard(context, onTap: () async {
              final permissionStatus = await Permission.camera.request();
              if (!permissionStatus.isGranted) {
                return;
              }

              showImageSourceDialog(context, (source) async {
                final permissionStatus = await Permission.camera.request();
                if (!permissionStatus.isGranted) return;

                await _pickTitleImage.pick(
                  pickMultiple: false,
                  context: context,
                  source: source,
                );

                _pickTitleImage.resumeSubscription(); // AFTER picking
                _pickTitleImage.pauseSubscription();
